...
revoke_cert_command = r'echo %node_name%'
...
class Balancer_Server(object):
    ...
    def remove(self, node_name, ip_addres, port):
        subprocess.call(revoke_cert_command.replace('%node_name%', node_name))

        haproxy_file = open('hproxy_config.cfg')
        haproxy_data = haproxy_file.read()
        haproxy_file.close()

        haproxy_data = haproxy_data.replace(serv_template.replace('%node_name%', node_name).replace('%ip%', ip_addres).replace('%port%', port), '')
        haproxy_file = open('hproxy_config.cfg', 'w')
        haproxy_file.write(haproxy_data)
        haproxy_file.close()
